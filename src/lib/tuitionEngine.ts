...
export function calculateStudentTuitionSummary(
  ...
  const studentClassIds = targetClassId ? [targetClassId] : (student.classIds || []);
  ...
  if (effectiveInvoices.length === 0) {
    ...
    effectiveInvoices = [
      {
        id: `legacy_${student.id}`,
        code: `LEGACY-${student.id.slice(-4).toUpperCase()}`,
  ...
  const minStartSessionNumber = Math.min(
    ...effectiveInvoices.map((inv) => Number(inv.startFromSessionNumber) || 1)
  );
