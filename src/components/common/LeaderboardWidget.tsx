...
export const LeaderboardWidget: React.FC<LeaderboardWidgetProps> = ({
  ...
  const content = (
    <div className="space-y-6">
      ...
      <div className="bg-slate-50/80 dark:bg-slate-800/50 p-6 rounded-2xl border border-slate-200/80 dark:border-slate-800 space-y-4">
        
        {/* Top Row: Icon, Main Title & Close Button */}
        <div className="flex items-center justify-between gap-4">
          <div className="flex items-center space-x-3">
            <div className="w-10 h-10 rounded-xl bg-amber-100 dark:bg-amber-950/60 text-amber-600 flex items-center justify-center font-bold text-lg border border-amber-200/60 shrink-0 shadow-2xs">
          ...
          {onClose && (
            <button
              onClick={onClose}
              className="p-1.5 rounded-lg bg-white dark:bg-slate-800 hover:bg-slate-100 text-slate-400 hover:text-slate-600 dark:hover:text-slate-200 transition border border-slate-200 dark:border-slate-700 shrink-0 cursor-pointer"
        ...
        <div className="pt-3 border-t border-slate-200/60 dark:border-slate-800 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          ...
          <div className="flex items-center space-x-2">
            ...
            <div className="bg-white dark:bg-slate-800 p-1 rounded-xl border border-slate-200 dark:border-slate-700 flex items-center space-x-1 shadow-2xs">
              <button
      ...
      <div className="space-y-3">
        {rankedStudents.map((item, index) => {
          ...
          return (
            <div
              key={item.student.id}
              className={`p-4 rounded-2xl border transition-all duration-150 flex flex-col sm:flex-row sm:items-center justify-between gap-4 shadow-2xs ${
                isTop1
                  ...
                  : isTop3
                  ? 'bg-orange-50/30 dark:bg-slate-800/30 border-orange-200/60 dark:border-slate-800'
            ...
            >
              <div className="flex items-center space-x-3.5">
                ...
                <div className="w-10 h-10 rounded-xl flex items-center justify-center font-bold text-sm shrink-0">
                  {isTop1 ? (
                    <div className="w-10 h-10 rounded-xl bg-amber-400 text-amber-950 flex items-center justify-center font-bold shadow-2xs text-base">
                      🥇
                    </div>
                  ) : isTop2 ? (
                    <div className="w-10 h-10 rounded-xl bg-slate-200 text-slate-800 flex items-center justify-center font-bold shadow-2xs text-base">
                      🥈
                    </div>
                  ) : isTop3 ? (
                    <div className="w-10 h-10 rounded-xl bg-amber-200 text-amber-900 flex items-center justify-center font-bold shadow-2xs text-base">
                      🥉
                    </div>
                  ) : (
                    <div className="w-9 h-9 rounded-lg bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 flex items-center justify-center font-semibold text-xs">
                      #{index + 1}
                    </div>
                  )}
                </div>
                ...
                <div>
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    {honorTitle && (
                      <span className="px-2.5 py-0.5 rounded-md text-[11px] font-medium bg-rose-50 text-rose-700 border border-rose-200/60 dark:bg-rose-950/40 dark:text-rose-300">
                        {honorTitle}
                      </span>
                    )}
                  </div>
              ...
              <div className="flex flex-wrap items-center gap-2 border-t sm:border-t-0 border-slate-100 dark:border-slate-800 pt-3 sm:pt-0 justify-end shrink-0">
                
                {/* METRIC 1: COMPLETION RATE */}
                <div className="px-3 py-1.5 rounded-xl bg-emerald-50 dark:bg-emerald-950/30 border border-emerald-200/60 text-center min-w-[95px]">
                  <span className="text-[10px] uppercase font-medium text-emerald-800 dark:text-emerald-400 block">
                    Hoàn Thành
                  </span>
                  <span className="text-xs font-semibold text-emerald-900 dark:text-emerald-200 flex items-center justify-center">
                ...
                <div className="px-3 py-1.5 rounded-xl bg-slate-50 dark:bg-slate-800/80 border border-slate-200/60 dark:border-slate-700 text-center min-w-[85px]">
                  <span className="text-[10px] uppercase font-medium text-slate-500 dark:text-slate-400 block">
                    Đã Làm
                  </span>
                  <span className="text-xs font-semibold text-slate-800 dark:text-slate-200 flex items-center justify-center">
                ...
                <div className="px-3 py-1.5 rounded-xl bg-amber-50 dark:bg-amber-950/30 border border-amber-200/60 text-center min-w-[95px]">
                  <span className="text-[10px] uppercase font-medium text-amber-800 dark:text-amber-400 block">
                    Chất Lượng
                  </span>
                  <span className="text-xs font-semibold text-amber-900 dark:text-amber-200 flex items-center justify-center">
